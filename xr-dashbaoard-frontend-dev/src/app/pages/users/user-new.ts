...
@Component({
    ...
    template: `
      <div class="max-w-8xl mx-auto p-4 bg-white rounded-lg shadow-md">
        <form class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6" (ngSubmit)="onSubmit()" #userForm="ngForm" novalidate>
          ...
          <div class="flex flex-col gap-1">
            <label for="employmentnumber" class="font-medium text-gray-800">Employee ID <span class="text-red-500">*</span></label>
            ...
            <div *ngIf="formSubmitted && !formData.EmploymentNumber" class="text-red-500 text-xs">Employee ID is required.</div>
          </div>
          <div class="flex flex-col gap-1">
...
export class NewUser implements OnInit {
        ...
        this.masterService.get_MDForUser().subscribe({
            ...
            error: () => {
                this.messageService.add({
                    severity: 'error',
                    summary: 'Error',
                    detail: 'Failed to load master data for user form'
                });
            }
        });
    }
    ...
    onSubmit() {
        ...
        this.userService.addUserNew(payload).subscribe({
            next: () => {
                this.messageService.add({
                    severity: 'success',
                    summary: 'Success',
